...
func TestString_Equal(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`strings not equal:
    got: (string) "0"
 expect: (string) "1"`)

	m.Reset()
	assert.ThatString(m, "0").Equal("1", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`strings not equal:
    got: (string) "0"
 expect: (string) "1"
...
func TestString_NotEqual(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`strings are equal:
    got: (string) "0"
 expect: not equal to "0"`)

	m.Reset()
	assert.ThatString(m, "0").NotEqual("0", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`strings are equal:
    got: (string) "0"
 expect: not equal to "0"
message: param (index=0)`)
...
func TestString_JSONEqual(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`invalid JSON in got value:
 ...
 expect: (string) "[{\"b\":1},{\"a\":0}]"
  error: invalid character 'h' in literal true (expecting 'r')`)
	...
	assert.ThatString(t, m.String()).Equal(`invalid JSON in expect value:
    got: (string) "{\"a\":0,\"b\":1}"
 expect: (string) "this is an error"
  error: invalid character 'h' in literal true (expecting 'r')`)
	...
	m.Reset()
	assert.ThatString(m, `{"a":0,"b":1}`).JSONEqual(`[{"b":1},{"a":0}]`)
	assert.ThatString(t, m.String()).Equal(`JSON structures are not equal:
    got: (string) "{\"a\":0,\"b\":1}"
 expect: (string) "[{\"b\":1},{\"a\":0}]"`)

	m.Reset()
	assert.ThatString(m, `{"a":0}`).JSONEqual(`{"a":1}`, "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`JSON structures are not equal:
    got: (string) "{\"a\":0}"
 expect: (string) "{\"a\":1}"
message: param (index=0)`)
...
func TestString_Matches(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string does not match the pattern:
    got: (string) "this is an error"
 ...
 expect: to match regex "an error"`)
	...
	m.Reset()
	assert.ThatString(m, "there's no error").Matches("an error", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string does not match the pattern:
    got: (string) "there's no error"
 expect: to match regex "an error"
message: param (index=0)`)
...
func TestString_EqualFold(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`strings are not equal under case-folding:
    got: (string) "hello, world!"
 expect: (string) "xxx"`)

	m.Reset()
	assert.ThatString(m, "hello, world!").EqualFold("xxx", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`strings are not equal under case-folding:
    got: (string) "hello, world!"
 expect: (string) "xxx"
message: param (index=0)`)
...
func TestString_HasPrefix(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string does not start with the specified prefix:
    got: (string) "hello, world!"
 expect: to have prefix "xxx"`)

	m.Reset()
	assert.ThatString(m, "hello, world!").HasPrefix("xxx", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string does not start with the specified prefix:
    got: (string) "hello, world!"
 expect: to have prefix "xxx"
message: param (index=0)`)
...
func TestString_HasSuffix(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string does not end with the specified suffix:
    got: (string) "hello, world!"
 expect: to have suffix "xxx"`)

	m.Reset()
	assert.ThatString(m, "hello, world!").HasSuffix("xxx", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string does not end with the specified suffix:
    got: (string) "hello, world!"
 expect: to have suffix "xxx"
message: param (index=0)`)
...
func TestString_Contains(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string does not contain the specified substring:
    got: (string) "hello, world!"
 expect: to contain substring "xxx"`)

	m.Reset()
	assert.ThatString(m, "hello, world!").Contains("xxx", "param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string does not contain the specified substring:
    got: (string) "hello, world!"
 expect: to contain substring "xxx"
message: param (index=0)`)
...
func TestString_IsEmpty(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is not empty:
    got: (string) "hello"
 expect: empty string`)

	m.Reset()
	assert.ThatString(m, "hello").IsEmpty("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is not empty:
    got: (string) "hello"
 expect: empty string
message: param (index=0)`)
...
func TestString_IsNotEmpty(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is empty:
    got: (string) ""
 expect: non-empty string`)

	m.Reset()
	assert.ThatString(m, "").IsNotEmpty("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is empty:
    got: (string) ""
 expect: non-empty string
message: param (index=0)`)
...
func TestString_IsBlank(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string contains non-whitespace characters:
    got: (string) "hello"
 expect: blank string`)

	m.Reset()
	assert.ThatString(m, "hello").IsBlank("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string contains non-whitespace characters:
    got: (string) "hello"
 expect: blank string
message: param (index=0)`)
...
func TestString_IsNotBlank(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is blank:
    got: (string) "   "
 expect: non-blank string`)

	m.Reset()
	assert.ThatString(m, "   ").IsNotBlank("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is blank:
    got: (string) "   "
 expect: non-blank string
message: param (index=0)`)
...
func TestString_IsLowerCase(t *testing.T) {
	...
	m.Reset()
	assert.ThatString(m, "Hello").IsLowerCase()
	assert.ThatString(t, m.String()).Equal(`string contains uppercase characters:
    got: (string) "Hello"
 expect: lowercase string`)

	m.Reset()
	assert.ThatString(m, "Hello").IsLowerCase("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string contains uppercase characters:
    got: (string) "Hello"
 expect: lowercase string
message: param (index=0)`)
...
func TestString_IsUpperCase(t *testing.T) {
	...
	m.Reset()
	assert.ThatString(m, "Hello").IsUpperCase()
	assert.ThatString(t, m.String()).Equal(`string contains lowercase characters:
    got: (string) "Hello"
 expect: uppercase string`)

	m.Reset()
	assert.ThatString(m, "Hello").IsUpperCase("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string contains lowercase characters:
    got: (string) "Hello"
 expect: uppercase string
message: param (index=0)`)
...
func TestString_IsNumeric(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string contains non-numeric characters:
    got: (string) "123a45"
 expect: numeric string`)

	m.Reset()
	assert.ThatString(m, "123a45").IsNumeric("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string contains non-numeric characters:
    got: (string) "123a45"
 expect: numeric string
message: param (index=0)`)
...
func TestString_IsAlpha(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string contains non-alphabetic characters:
    got: (string) "abc123"
 expect: alphabetic string`)

	m.Reset()
	assert.ThatString(m, "abc123").IsAlpha("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string contains non-alphabetic characters:
    got: (string) "abc123"
 expect: alphabetic string
message: param (index=0)`)
...
func TestString_IsAlphaNumeric(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string contains non-alphanumeric characters:
    got: (string) "abc@123"
 expect: alphanumeric string`)

	m.Reset()
	assert.ThatString(m, "abc@123").IsAlphaNumeric("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string contains non-alphanumeric characters:
    got: (string) "abc@123"
 expect: alphanumeric string
message: param (index=0)`)
...
func TestString_IsEmail(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is not a valid email:
    got: (string) "invalid-email"
 expect: valid email address`)

	m.Reset()
	assert.ThatString(m, "invalid-email").IsEmail("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is not a valid email:
    got: (string) "invalid-email"
 expect: valid email address
message: param (index=0)`)
...
func TestString_IsURL(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is not a valid URL:
    got: (string) "invalid-url"
 expect: valid URL`)

	m.Reset()
	assert.ThatString(m, "invalid-url").IsURL("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is not a valid URL:
    got: (string) "invalid-url"
 expect: valid URL
message: param (index=0)`)
...
func TestString_IsIP(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is not a valid IP:
    got: (string) "invalid-ip"
 expect: valid IP address`)

	m.Reset()
	assert.ThatString(m, "invalid-ip").IsIP("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is not a valid IP:
    got: (string) "invalid-ip"
 expect: valid IP address
message: param (index=0)`)
...
func TestString_IsHex(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is not a valid hexadecimal:
    got: (string) "abcdefg"
 expect: valid hexadecimal number`)

	m.Reset()
	assert.ThatString(m, "abcdefg").IsHex("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is not a valid hexadecimal:
    got: (string) "abcdefg"
 expect: valid hexadecimal number
message: param (index=0)`)
...
func TestString_IsBase64(t *testing.T) {
	...
	assert.ThatString(t, m.String()).Equal(`string is not a valid Base64:
    got: (string) "invalid-base64"
 expect: valid Base64 encoded string`)

	m.Reset()
	assert.ThatString(m, "invalid-base64").IsBase64("param (index=0)")
	assert.ThatString(t, m.String()).Equal(`string is not a valid Base64:
    got: (string) "invalid-base64"
 expect: valid Base64 encoded string
message: param (index=0)`)
